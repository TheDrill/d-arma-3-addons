...
GVAR(SCommandMarkersLogic) = "Logic" createVehicleLocal [-10000,-10000,0];
...
RC_DEFINE(SAddLineMarker) =
{
	...
	{
		PV(_j) = GVAR(SCommandMarkersLogic) getVariable [ str MAR_CHANDATA(_this), objNull ];
		if (isNull _j) then
		{
			_j = "Logic" createVehicleLocal [-10000,-10000,0];
			 GVAR(SCommandMarkersLogic) setVariable [ str MAR_CHANDATA(_this), _j ];
		};
		...
		if (!isDedicated && {playerSide == MAR_CHANDATA(_this) && 
				{!isNull player && {alive player && {leader (group player) == player}}}
			}) then
		{
			_this call RC_FUNC(CAddLineMarker);
		};
		...
		{
			if ( ( isPlayer _x || {time <= 0 && (owner _x) > 2} ) && 
				{ _x != player && { { side (group _x) == MAR_CHANDATA(_this) } &&
					{ leader (group _x) == _x } }
				}) then
			{
				#define RC_EXEC_ARG _this
...
RC_DEFINE(SDelLineMarker) =
{
	...
	if ((_this select 1) == CHAN_COMMAND) then
	{
		_j = GVAR(SCommandMarkersLogic) getVariable 
			[str (_this select 2), objNull];
	};
		...
		{!isNull _j && {count (_j getVariable [str (_this select 0), []]) > 0}} ) then
	{
		...
		if ((_this select 1) == CHAN_COMMAND) then
		{
			if (!isDedicated && {playerSide == _this select 2 &&
					{alive player && {leader (group player) == player}} }) then
			{
				(_this select 0) call RC_FUNC(CDelLineMarker);
			};
			...
			{
				if (( isPlayer _x || {time <= 0 && (owner _x) > 2} ) && { _x != player && 
					{side (group _x) == _this select 2 && {leader (group _x) == _x}} }) then
				{
					#define RC_EXEC_ARG _this select 0
					RC_EXEC_C(CDelLineMarker, owner _x);
				};
...
RC_DEFINE(SRequestMarkersForPlayer) =
{
	_this spawn
	{
		if (!isNull _this) then
		{
			...
			PV(_y) = GVAR(SCommandMarkersLogic) getVariable [str (side (group _this)), objNull];
			if (!isNull _y && (leader (group _this) == _this)) then
			{
				[_pid, _y] call FUNC(S__SendAllMarkers);
			};
