...
FUNC(UIUpdateMarkersParams) =
{
	for "_x" from 0 to ((count GVAR(UIPoints)) - 1) do
	{
		...
		if (_x > 0) then
		{
			M_TEMP_MARKER(_x) setMarkerSizeLocal [ GVAR(UIMarkerThick),
...
// hotkey
	...
	if ((_this select 1 == 28 || _send_to_channel >= 0) && count GVAR(UIPoints) > 0) then
	{
		if (count GVAR(UIPoints) > 1) then
		{
			...
			_val = [-1, _chan, _chan call FUNC(UIgetPlayerChanData),
				...
				GVAR(UIMarkerColor), GVAR(UIMarkerThick), GVAR(UIPoints)];
	...
	GVAR(UILastKEHRVState) = _force_handled ||
		( (count GVAR(UIPoints) > 0) && ((_this select 1) in [200, 208, 203, 205, 14, 28, 1]) );
	GVAR(UILastKEHRVState);
};
