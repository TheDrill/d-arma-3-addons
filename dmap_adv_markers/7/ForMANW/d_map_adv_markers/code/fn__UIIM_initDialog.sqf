...
PV(_shift) = 0 min ( safeZoneY + safeZoneH - ((_pos select 1) + (_pos select 3) * 4) );
...
_description ctrlsetstructuredtext parsetext format 
	["<t size='0.8'>%1</t> <t align='right' size='0.8'>(%2)</t>",
	localize "STR_Addons__d_map_adv_markers__IMD_Description", GVAR(str_localized_cur_channel)]; //--- ToDo: Localze
...
//--- ButtonOK
_pos set [1,_posY + 3 * _posH + 2 * BORDER];
