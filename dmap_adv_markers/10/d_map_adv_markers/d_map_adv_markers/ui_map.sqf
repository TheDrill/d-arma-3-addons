...
FUNC(UIGetMapScaleRelatedMarkerThickness) = 
{
	...
	PV(_top) = _mctrl ctrlMapScreenToWorld [0.5, safeZoneY];
	PV(_bottom) = _mctrl ctrlMapScreenToWorld [0.5, safeZoneY + SafeZoneH];
	
	PV(_rng) = (_top select 1) - (_bottom select 1);
...
{
	PV(_scale) = [] call FUNC(UIGetMapScaleRelatedMarkerThickness);
...
// hotkey
	...
	if ((_this select 1 == 28 || _send_to_channel >= 0) && count GVAR(UIPoints) > 0) then
	{
		if (count GVAR(UIPoints) > 1) then
		{
			...
			if ([_val] call CFUNC(_CCheckIfMarkerAllowedSend)) then
			{
				_val call FUNC(CAddLineMarker_Temporary);				
					
				#define RC_EXEC_ARG [_val, player]
