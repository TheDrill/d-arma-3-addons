...
GVAR(CMarkerTagDaytimeVisibility) = MTAG_DEFAULT_OPTIONS select 2;
GVAR(CLineMarkerTagNameVisibility) = MTAG_DEFAULT_OPTIONS select 3;
GVAR(CLineMarkerTagChannelVisibility) = MTAG_DEFAULT_OPTIONS select 4;
GVAR(CLineMarkerTagDaytimeVisibility) = MTAG_DEFAULT_OPTIONS select 5;
...
if (!isNil "d_uac_fnc_registerParameter") then
{
...

	{
		[_x select 0, _sect_name, localize (_x select 1), "", _enum_values] 
			...
			['GVAR(CMarkerTagDaytimeVisibility)',
				"STR_Addons__d_map_adv_markers__uac_MarkerTagDaytimeVisibility"],
			...
			['GVAR(CLineMarkerTagDaytimeVisibility)',
				"STR_Addons__d_map_adv_markers__uac_LineMarkerTagDaytimeVisibility"]
		];
};
...
FUNC(CapplyMarkerVisibility) =
{
	...
	if (count MAR_COORDS(_mar) > 1) then
	{
		...
		{
			PV(_dt) = MAR_DAYTIME(_mar);

			PV(_h) = floor _dt;
			PV(_ht) = str _h;
			if ((count _ht) < 2) then {_ht = "0" + _ht;};
			
			PV(_m) = floor ((_dt - _h) * 60);
			PV(_mt) = str _m;
...
RC_DEFINE(CAddLineMarker) = 
{
	...
	PV(_marker_allowed) = true;
	
	if (_this select 1) then
	{
		_marker_allowed = [_mark, _this select 2] call 
			CFUNC(_CCheckIfMarkerAllowedReceive);
		
		if (_marker_allowed) then
		{
			#define RC_EXEC_ARG [player, MAR_ID(_mark), MAR_UID(_mark)]
			RC_EXEC_S(SStorePlayerSeenMarker);
		};
	};
	...
	if ( _mark call FUNC(C_isMarkerVisibleToPlayer) ) then
	{
		...
		PV(_col) = MAR_COLOR(_mark);
		PV(_th) = MAR_THICKNESS(_mark);
